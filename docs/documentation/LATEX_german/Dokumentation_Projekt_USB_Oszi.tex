...
Ereignisse werden beim Auftreten lediglich \emph{eingereiht} (\texttt{enqueueEvent()}) und später von der FSM sequenziell \emph{abgearbeitet} (\texttt{dequeueEvent()}).\\
...
\textbf{\emph{Theoretischer Bezug}}\\
Die Queue bildet die Schnittstelle zum Ereignisalphabet $E$ der FSM und sorgt dafür, dass Ereignisse in der Reihenfolge ihres Eintreffens verarbeitet werden. 
Dies entspricht der Theorie eines deterministischen endlichen Automaten (DEA) und unterstützt einen klar definierten, vorhersehbaren Ablauf.
